...
def ledger_handler(guarantors, amount, chat_id):
    ...
    sender_user = Users.query.filter_by(telegram_id=chat_id).first()
    sender_name = sender_user.username

    if sender_name not in guarantors:
        raise ValueError(f'Вы не указали себя среди пользователей')

    usernames = [user.username for user in all_users]
    amount = float(amount) / len(guarantors)

    for name in guarantors:
        if name not in usernames:
            raise ValueError(f'Имя {name} отстуствует в таблице пользователей!')
...
def drop_balance(update, context):
    all_users = Users.query.all()
    for user in all_users:
        user = Users.query.filter_by(username=user.username).first()
        user.balance = 0
    update.message.reply_text(f'Баланс обнулен у всех участников!')
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.text & ~Filters.command & Filters.regex(r'^(\d{1,6})\s(\D{1,4})'),
                                  add_payment))
    dp.add_handler(CommandHandler(command="drop_balance",callback=drop_balance, filters=Filters.user(233133830)))
